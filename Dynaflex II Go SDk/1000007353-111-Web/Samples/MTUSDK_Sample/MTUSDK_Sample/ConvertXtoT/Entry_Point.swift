...
class Entry_Point: TLVfileWithDelmiters{
    ...
    init(sourceFile: String, inputCfgName: String, version: String, destinationFolder : String, _version: UInt8 = 0xAA){
        ...
        writeLog(lt: LogType.INFO, s: "ENTRY_POINT Cstor called. Version = \(String(format:"%02X", _version)), OutFile = \(OutFile)")
    }
    ...
    init(sourceFile: String, _version: UInt8 = 0xAA) {
        ...
        writeLog(lt: LogType.INFO, s: "ENTRY_POINT Cstor called. Version = \(String(format:"%02X", _version))")
    }
    
    func getFileID() ->Int {
        
        return FileID().EMV_CONFIGURATION_FI_ENTRY_POINT
            ...
            for wbk in try file.parseWorkbooks() {
              for (name, path) in try file.parseWorksheetPathsAndNames(workbook: wbk) {
                  if let sCurrSection = name {
                      if (sCurrSection.uppercased().contains("ENTRYPOINT")){
...
//                                }
                                ...
                                if (contentNumber.hasPrefix("C")){
                                    ...
                                    if Int(contentIndex)! > titleRow {
                                        ...
                                        if temp1.count > 0 {
                                            ...
                                            let temp = UInt32(temp1, radix: 16)!
                          ...
                          let tlv_FF35: TLV = TLV.init(tag: "FF35", value: "")
                          addTLVItem(tlv: tlv_FF35)
                      }
                  }
              }
                if bHasData {
