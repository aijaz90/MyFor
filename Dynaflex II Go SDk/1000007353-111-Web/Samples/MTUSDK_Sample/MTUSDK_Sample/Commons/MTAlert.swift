...
//  ReaderConfig
//
//  Created by Tam Nguyen on 12/2/15.
...
let MTAlertActionEnabledDidChangeNotification = "MTAlertActionEnabledDidChangeNotification"

enum MTAlertActionStyle : Int {
    case `default`
    case cancel
    case destructive
}

enum MTAlertControllerStyle : Int {
    case actionSheet
    case alert
}

// MARK: MTAlertAction Class

class MTAlertAction : NSObject, NSCopying {
    var title: String
    var style: MTAlertActionStyle
    var handler: ((MTAlertAction?) -> Void)!
    @objc var enabled: Bool {
        didSet {
            if (oldValue != enabled) {
                NotificationCenter.default.post(name: Notification.Name(rawValue: MTAlertActionEnabledDidChangeNotification), object: nil)
            }
        }
    }
    
    required init(title: String, style: MTAlertActionStyle, handler: ((MTAlertAction?) -> Void)!) {
        self.title = title
        self.style = style
...
// MARK: MTAlertAnimation Class

@available(iOS 13.0, *)
class MTAlertAnimation : NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func presentAnimateTransition(_ transitionContext: UIViewControllerContextTransitioning) {
        ...
        let alertController = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.to) as! MTAlertController
    ...
    func dismissAnimateTransition(_ transitionContext: UIViewControllerContextTransitioning) {
        ...
        let alertController = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.from) as! MTAlertController
...
// MARK: MTAlertController Class

@available(iOS 13.0, *)
class MTAlertController : UIViewController, UITextFieldDelegate, UIViewControllerTransitioningDelegate {
    
    // Message
    var message: String?
    
    // AlertController Style
    fileprivate(set) var preferredStyle: MTAlertControllerStyle?
    ...
    var titleFont = UIFont(name: "HelveticaNeue-Thin", size: 18)
    var titleTextColor = UIColor(red:77/255, green:77/255, blue:77/255, alpha:1.0)
    
    // MessageView
    fileprivate var messageView = UILabel()
    var messageFont = UIFont(name: "HelveticaNeue-Light", size: 15)
    ...
    var buttonFont: [MTAlertActionStyle : UIFont?] = [
        .default : UIFont(name: "HelveticaNeue-Light", size: 16),
        .cancel  : UIFont(name: "HelveticaNeue-Light", size: 16),
        .destructive  : UIFont(name: "HelveticaNeue-Thin", size: 16)
    ]
    var buttonTextColor: [MTAlertActionStyle : UIColor] = [
        .default : UIColor.white,
        .cancel  : UIColor.white,
        .destructive  : UIColor.white
    ]
    var buttonBgColor: [MTAlertActionStyle : UIColor] = [
    ...
    var buttonBgColorHighlighted: [MTAlertActionStyle : UIColor] = [
    ...
    // Initializer
    convenience init(title: String?, message: String?, preferredStyle: MTAlertControllerStyle) {
        ...
        // NotificationCenter
        NotificationCenter.default.addObserver(self, selector: #selector(MTAlertController.handleAlertActionEnabledDidChangeNotification(_:)), name: NSNotification.Name(rawValue: MTAlertActionEnabledDidChangeNotification), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MTAlertController.handleKeyboardWillShowNotification(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MTAlertController.handleKeyboardWillHideNotification(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if (!isAlert() && cancelButtonTag != 0) {
            let tapGesture = UITapGestureRecognizer(target: self, action: #selector(MTAlertController.handleContainerViewTapGesture(_:)))
    ...
    func layoutView() {
        ...
        if (isAlert() && buttons.count == 2) {
            ...
            for button in buttons {
                let action = actions[button.tag - 1] as! MTAlertAction
            ...
            for button in buttons {
                let action = actions[button.tag - 1] as! MTAlertAction
                if (action.style != MTAlertActionStyle.cancel) {
                    button.titleLabel?.font = buttonFont[action.style]!
                    button.setTitleColor(buttonTextColor[action.style], for: UIControl.State())
                    button.setBackgroundImage(createImageFromUIColor(buttonBgColor[action.style]!), for: UIControl.State())
                    button.setBackgroundImage(createImageFromUIColor(buttonBgColorHighlighted[action.style]!), for: .highlighted)
            ...
            if (cancelButtonTag != 0) {
                ...
                let action = actions[cancelButtonTag - 1] as! MTAlertAction
                button.titleLabel?.font = buttonFont[action.style]!
                button.setTitleColor(buttonTextColor[action.style], for: UIControl.State())
                button.setBackgroundImage(createImageFromUIColor(buttonBgColor[action.style]!), for: UIControl.State())
                button.setBackgroundImage(createImageFromUIColor(buttonBgColorHighlighted[action.style]!), for: .highlighted)
    ...
    @objc func buttonTapped(_ sender: UIButton) {
        ...
        let action = actions[sender.tag - 1] as! MTAlertAction
    ...
    @objc func handleContainerViewTapGesture(_ sender: AnyObject) {
        // cancel action
        let action = actions[cancelButtonTag] as! MTAlertAction
        if (action.handler != nil) {
            action.handler(action)
        }
        self.dismiss(animated: true, completion: completionHandler)
    ...
    func addAction(_ action: MTAlertAction) {
        // Error
        if (action.style == MTAlertActionStyle.cancel) {
            for ac in actions as! [MTAlertAction] {
                if (ac.style == MTAlertActionStyle.cancel) {
                    //let error: NSError?
                    //NSException.raise("NSInternalInconsistencyException", format:"MTAlertController can only have one action with a style of MTAlertActionStyleCancel", arguments:getVaList([error ?? "nil"]))
                    return
                }
            }
        }
        ...
        button.addTarget(self, action: #selector(MTAlertController.buttonTapped(_:)), for: .touchUpInside)
        button.tag = buttons.count + 1
        buttons.append(button)
        buttonContainer.addSubview(button)
    }
    
    func addDismissedBlock(_ completion:@escaping (() -> Void))
    {
    ...
    func addTextFieldWithConfigurationHandler(_ configurationHandler: ((UITextField?) -> Void)!) {
        ...
        // You can add a text field only if the preferredStyle property is set to MTAlertControllerStyle.Alert.
        if (!isAlert()) {
            //let error: NSError?
            //NSException.raise("NSInternalInconsistencyException", format: "Text fields can only be added to an alert controller of style MTAlertControllerStyleAlert", arguments:getVaList([error ?? "nil"]))
            return
        }
        if (textFields == nil) {
            textFields = []
    ...
    func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        layoutView()
        return MTAlertAnimation(isPresenting: true)
    }
    
    func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        return MTAlertAnimation(isPresenting: false)
    }
}
    ...
    static var isLandscape: Bool {
        if #available(iOS 13.0, *) {
            if #available(iOS 15.0, *) {
                return (UIApplication.shared.connectedScenes.first as? UIWindowScene)?.windows.first?.windowScene?.interfaceOrientation.isLandscape ?? false
            } else {
