...
enum CocoaMQTTLoggerLevel : NSInteger;
enum CocoaMQTTQoS : uint8_t;
/// MQTT Client
/// note:
/// MGCDAsyncSocket need delegate to extend NSObject
SWIFT_CLASS("_TtC9CocoaMQTT9CocoaMQTT")
@interface CocoaMQTT : NSObject
@property (nonatomic, weak) id <CocoaMQTTDelegate> _Nullable delegate;
...
/// todo:
/// What’s behavior each Clean Session flags???
@property (nonatomic) BOOL cleanSession;
/// Setup a <em>Last Will Message</em> to client before connecting to broker
@property (nonatomic, strong) CocoaMQTTMessage * _Nullable willMessage;
/// Enable backgounding socket if running on iOS platform. Default is true
/// note:
///
@property (nonatomic) BOOL backgroundOnSocket;
/// Delegate Executed queue. Default is <code>DispatchQueue.main</code>
...
@property (nonatomic) enum CocoaMQTTConnState connState;
/// Re-deliver the un-acked messages
@property (nonatomic) double deliverTimeout;
...
/// note:
/// This value will be increased with <code>autoReconnectTimeInterval *= 2</code>
/// if reconnect failed
@property (nonatomic) uint16_t autoReconnectTimeInterval;
/// Maximum auto reconnect time interval
/// The timer starts from <code>autoReconnectTimeInterval</code> second and grows exponentially until this value
/// After that, it uses this value for subsequent requests.
@property (nonatomic) uint16_t maxAutoReconnectTimeInterval;
/// Console log level
@property (nonatomic) enum CocoaMQTTLoggerLevel logLevel;
/// Enable SSL connection
@property (nonatomic) BOOL enableSSL;
///
@property (nonatomic, copy) NSDictionary<NSString *, NSObject *> * _Nullable sslSettings;
/// Allow self-signed ca certificate.
/// Default is false
@property (nonatomic) BOOL allowUntrustCACertificate;
...
- (nonnull instancetype)initWithClientID:(NSString * _Nonnull)clientID wsUrl:(NSString * _Nonnull)wsUrl port:(uint16_t)port OBJC_DESIGNATED_INITIALIZER;
...
///   </li>
/// </ul>
- (BOOL)connectWithTimeout:(NSTimeInterval)timeout SWIFT_WARN_UNUSED_RESULT;
...
/// \param topic Topic Name. It can not contain ‘#’, ‘+’ wildcards
///
/// \param string Payload string
///
/// \param qos Qos. Default is Qos1
///
/// \param retained Retained flag. Mark this message is a retained message. default is false
///
///
/// returns:
///
...
///   </li>
/// </ul>
- (NSInteger)publish:(NSString * _Nonnull)topic withString:(NSString * _Nonnull)string qos:(enum CocoaMQTTQoS)qos retained:(BOOL)retained;
/// Publish a message to broker
/// \param message Message
///
- (NSInteger)publish:(CocoaMQTTMessage * _Nonnull)message;
/// Subscribe a <code><Topic Name>/<Topic Filter></code>
/// \param topic Topic Name or Topic Filter
///
/// \param qos Qos. Default is qos1
///
- (void)subscribe:(NSString * _Nonnull)topic qos:(enum CocoaMQTTQoS)qos;
/// Unsubscribe a Topic
/// \param topic A Topic Name or Topic Filter
///
- (void)unsubscribe:(NSString * _Nonnull)topic;
...
@class MqttConnectProperties;
@class MqttAuthProperties;
enum CocoaMQTTDISCONNECTReasonCode : uint8_t;
@class MqttPublishProperties;
...
@property (nonatomic, weak) id <CocoaMQTT5Delegate> _Nullable delegate;
@property (nonatomic, copy) NSString * _Nonnull host;
@property (nonatomic) uint16_t port;
@property (nonatomic, copy) NSString * _Nonnull clientID;
@property (nonatomic, copy) NSString * _Nullable username;
...
@property (nonatomic) BOOL cleanSession;
/// Setup a <em>Last Will Message</em> to client before connecting to broker
@property (nonatomic, strong) CocoaMQTT5Message * _Nullable willMessage;
/// Enable backgounding socket if running on iOS platform. Default is true
/// note:
///
@property (nonatomic) BOOL backgroundOnSocket;
...
@property (nonatomic) NSUInteger messageQueueSize;
/// In-flight window size. default 10
@property (nonatomic) NSUInteger inflightWindowSize;
/// Keep alive time interval
@property (nonatomic) uint16_t keepAlive;
/// Enable auto-reconnect mechanism
@property (nonatomic) BOOL autoReconnect;
...
@property (nonatomic, strong) MqttConnectProperties * _Nullable connectProperties;
/// 3.15.2.2 AUTH Properties
@property (nonatomic, strong) MqttAuthProperties * _Nullable authProperties;
/// Console log level
@property (nonatomic) enum CocoaMQTTLoggerLevel logLevel;
/// Enable SSL connection
@property (nonatomic) BOOL enableSSL;
...
/// disconnect expectedly
- (void)disconnect;
- (void)disconnectWithReasonCode:(enum CocoaMQTTDISCONNECTReasonCode)reasonCode userProperties:(NSDictionary<NSString *, NSString *> * _Nonnull)userProperties;
/// Send a PING request to broker
- (void)ping;
/// Publish a message to broker
/// \param topic Topic Name. It can not contain ‘#’, ‘+’ wildcards
...
///   </li>
/// </ul>
- (NSInteger)publish:(NSString * _Nonnull)topic withString:(NSString * _Nonnull)string qos:(enum CocoaMQTTQoS)qos DUP:(BOOL)DUP retained:(BOOL)retained properties:(MqttPublishProperties * _Nonnull)properties;
/// Publish a message to broker
/// \param message Message
///
/// \param properties Publish Properties
///
- (NSInteger)publish:(CocoaMQTT5Message * _Nonnull)message DUP:(BOOL)DUP retained:(BOOL)retained properties:(MqttPublishProperties * _Nonnull)properties;
/// Subscribe a <code><Topic Name>/<Topic Filter></code>
/// \param topic Topic Name or Topic Filter
///
/// \param qos Qos. Default is qos1
...
- (void)authWithReasonCode:(enum CocoaMQTTAUTHReasonCode)reasonCode authProperties:(MqttAuthProperties * _Nonnull)authProperties;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
@property (nonatomic) enum CocoaMQTTQoS qos;
...
/// The <code>duplicated</code> property show that this message maybe has be received before
/// note:
/// Readonly property
@property (nonatomic) BOOL duplicated;
/// 3.1.3.2.3 Payload Format Indicator
@property (nonatomic) BOOL isUTF8EncodedData;
/// 3.1.3.2.5 Content Type
@property (nonatomic, copy) NSString * _Nullable contentType;
/// 3.1.3.2.6 Response Topic
@property (nonatomic, copy) NSString * _Nullable willResponseTopic;
/// 3.1.3.2.7 Correlation Data
@property (nonatomic, copy) NSArray<NSNumber *> * _Nullable willCorrelationData;
/// 3.1.3.2.8 User Property
@property (nonatomic, copy) NSDictionary<NSString *, NSString *> * _Nullable willUserProperty;
...
@property (nonatomic, readonly, copy) NSArray<NSNumber *> * _Nonnull properties;
- (nonnull instancetype)initWithTopic:(NSString * _Nonnull)topic string:(NSString * _Nonnull)string qos:(enum CocoaMQTTQoS)qos retained:(BOOL)retained OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithTopic:(NSString * _Nonnull)topic payload:(NSArray<NSNumber *> * _Nonnull)payload qos:(enum CocoaMQTTQoS)qos retained:(BOOL)retained OBJC_DESIGNATED_INITIALIZER;
- (nullable instancetype)initWithTopic:(NSString * _Nonnull)topic payload:(NSDictionary<NSString *, id> * _Nonnull)payload qos:(enum CocoaMQTTQoS)qos retained:(BOOL)retained error:(NSError * _Nullable * _Nullable)error OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
typedef SWIFT_ENUM(NSInteger, CocoaMQTTLoggerLevel, open) {
  CocoaMQTTLoggerLevelDebug = 0,
  CocoaMQTTLoggerLevelInfo = 1,
  CocoaMQTTLoggerLevelWarning = 2,
  CocoaMQTTLoggerLevelError = 3,
  CocoaMQTTLoggerLevelOff = 4,
};
...
- (nonnull instancetype)initWithContentType:(NSString * _Nonnull)contentType OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
